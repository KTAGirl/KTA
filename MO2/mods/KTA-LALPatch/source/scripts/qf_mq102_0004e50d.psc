...
Alias_Alduin.GetReference().Moveto(AlduinStartMarker)
Alias_Alduin.GetActorReference().SetActorValue("Aggression", 0)
Alias_Alduin.GetReference().Enable()
; if player doesn't yet have Riverwood objective, make sure friend forcegreets
if GetStageDone(10) == 0 && Alias_FactionFriend.GetReference()
	if Alias_FactionFriend.GetReference().IsInInterior()
		if Alias_FactionFriend.GetReference().Is3DLoaded() == 0
; 			debug.trace(self + " move friend to exterior")
			Alias_FactionFriend.TryToMoveTo(ChargenExitMarker)
		endif
	endif
	Alias_FactionFriend.TryToEvaluatePackage()
endif
;END CODE
EndFunction
;END FRAGMENT
...
;BEGIN CODE
ARTHLALRumorsOfWarQuest.GerdurTold = 1
;END CODE
EndFunction
;END FRAGMENT
...
;BEGIN CODE
ARTHLALRumorsOfWarQuest.AlvorTold = 1
;END CODE
EndFunction
;END FRAGMENT
